	...
	skyVS(skyVS)
{
	...
	// Create the cubemap
	cubeMap = CreateCubemap(right, left, up, down, front, back);
}
...
void Sky::Draw(std::shared_ptr<Camera> camera)
{
	...
	// Set srv and sampler
	skyPS->SetShaderResourceView("SkyTexture", cubeMap);
...
Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> Sky::GetCubeMap() { return cubeMap; }
