...
Mesh::Mesh (const char* name, Vertex vertices[], size_t verticesSize, unsigned int indices[], size_t indicesSize) : name(name)
{
	...
	CreateBuffers(vertices, verticesSize, indices, indicesSize);
}

Mesh::Mesh(const char* name, const char* fileName) : name(name)
{
	...
	obj.close();
	// END CODE PROVIDED FOR ASSIGNMENT ********************************************************************
...
{
	// Calculate our tangents before the buffers
